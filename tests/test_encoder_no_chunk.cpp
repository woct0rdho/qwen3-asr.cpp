...
int main() {
    ...
    if (!load_npy_f32("tests/reference/encoder_no_chunk.npy", ref_data, ref_shape)) {
    ...
    if (!encoder.encode_no_chunk(mel_data.data(), n_mel, n_frames, output)) {
        fprintf(stderr, "Failed to encode: %s\n", encoder.get_error().c_str());
        return 1;
    }
    
    int64_t out_seq = output.size() / 1024;
    printf("Output shape: [%lld, 1024]\n", (long long)out_seq);
    printf("Reference shape: [%lld, %lld]\n", (long long)ref_shape[0], (long long)ref_shape[1]);
